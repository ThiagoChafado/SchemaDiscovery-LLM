    ...
    for schemaFilename in os.listdir(PROCESSED_SCHEMAS_DIR):
        if schemaFilename.endswith('.schema.json'):
            ...
            if os.path.exists(jsonFilePath):
                relativeJsonPath = os.path.join(os.path.basename(DATASETS_DIR), os.path.basename(RAW_JSON_DIR), jsonFilename)
                relativeSchemaPath = os.path.join(os.path.basename(DATASETS_DIR), os.path.basename(PROCESSED_SCHEMAS_DIR), schemaFilename)
...
def generateSchemasAutomatically():
    """
    Generates schemas for new JSON files and updates the manifest upon completion.
    """
    ...
    for filename in jsonFiles:
        ...
        # --- NEW: Check if the schema file already exists ---
